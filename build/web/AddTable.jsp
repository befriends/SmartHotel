...
<html>
    <head>
        ...
        <script type="text/javascript" lang="javascript">
            function submitUpdateForm(cnt) {
                var f = "form" + cnt;
                document.getElementById(f).action = "UpdateTable.jsp?act=2";
                document.getElementById(f).submit();
            }

            function submitDeleteForm(cnt) {
                var f = "form" + cnt;
                document.getElementById(f).action = "TableController?act=3";
                document.getElementById(f).submit();
            }
   ...
   <div class="main_container" style="height: 600px;">
            ...
            <div class="container">
                ...
                <div class="container-fluid">
                    ...
                    <div style="margin-left: auto; margin-right: auto; width: 80%; background-color: antiquewhite;">
                        ...
                        %><div class="alert alert-danger fade in">
                            ...
                            params.put("submodule", "tabledetails"); // Database Table Name
                            params.put("columnname", "tableid"); // Database Column Name
                        ...
                        <span class="label label-info center-block" style="height:30px;font-size:20px;font-weight:bolder;vertical-align:middle;">Table</span>
                        <div>
                            <fieldset style="border:1px solid silver; padding:5px;">
                                <form role="form" class="form-horizontal" action="TableController" method="post">
                                    <div class="form-group">
                                        <label for="tableid" class="control-label col-sm-2">Table ID: </label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="tableid" name="tableid" value="<%=id%>" readonly="true" />
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="tablenm" class="control-label col-sm-2">Table Name: </label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="tablenm" placeholder="Enter Table Name" name="tablename" />
                                        </div>
                                    </div><div class="form-group">
                                        <label for="tableno" class="control-label col-sm-2">Table Number: </label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="tableno" placeholder="Enter Table Number" name="tablenumber" />
                                        </div>
                                    </div>

                                    <div class="form-group"> 
                                        <div class="col-sm-offset-2 col-sm-5">
                                            <button type="submit" name="submit" value="Add" class="btn btn-default">Add Table</button>
                ...
                <% 
                    JSONObject tableJSONObject = tableDaoObj.getAllTableList();
                    JSONArray itemarr = new JSONArray();
                    if (tableJSONObject != null && tableJSONObject.has("success") && tableJSONObject.has("data")) {
                        itemarr = tableJSONObject.getJSONArray("data");
                }%>
		...
		<tbody>
                ...
                for(cnt=0; cnt < itemarr.length(); cnt++){
                    JSONObject tableObj = itemarr.getJSONObject(cnt);
                %>
		<tr class="info" id="editrecordid<%=cnt %>" style="display:none;">
		<form name="editrecords">
		<td style="display: none;">
                    <label id="tableid" style="display: none;"><%=tableObj.getLong("tableid")%></label>
                    <label id="submodule" style="display: none;">table</label>
                    
                </td>
		<td><label><%=cnt+1 %></label></td>
		<td><input type="text" name="tablename" value="<%=tableObj.get("tablename")%>"/></td>
		<td>
		...
		<td><label><%=cnt+1 %></label></td>
		<td><label name="tablename"><%=tableObj.get("tablename")%></label></td>
                <td>
